"""Test large GEMM distributed across 2 groups of 4 Q32 cores.

Runs a 1x4096 @ 4096x4096 GEMM on a 512x512 int4 tensor core config.
Output tiles along N: 4096 / 512 = 8 subgraphs, round-robin across 8 Q32 cores.

Layout:
  Group 0: Q32 cores [0,1,2,3] + DSP core 4
  Group 1: Q32 cores [5,6,7,8] + DSP core 9

Expected with q32_2group_local_dram_dsp.json config (512-lane, int4):
- 8 subgraphs, 1 per Q32 core (8 Q32 cores across 2 groups)
- Reads: local DRAM (902 cycles each) -- bypass NoC
- Writes: core-to-core transfers over NoC to LOCAL DSP only
  - Cores 0-3 write to DSP 4
  - Cores 5-8 write to DSP 9

Usage:
    TOGSIM_CONFIG=configs/q32_2group_local_dram_dsp.json python tests/test_q32_2group_gemm.py
"""
import os
import sys
import glob
...
# ── Test: Large GEMM (1x4096 @ 4096x4096) across 2 groups, 8 Q32 cores ──
import json
...
print("\n" + "=" * 70)
print(f"Test: Large GEMM ({_M}x{_K} @ {_K}x{_N}) — {_num_q32}-core round-robin (2 groups)")
print("=" * 70)
...
if _q32_groups:
    for i, g in enumerate(_q32_groups):
        print(f"  Group {i}: Q32 cores {g['q32_cores']} -> DSP {g['dsp_core']}")
print()
...
Memory access:
  ...
  - Group isolation: Q32 cores write ONLY to their local DSP
...
K-reduction loop (inner):
  ...
  DMA Y_buffer -> output via NoC to LOCAL DSP core
""")
